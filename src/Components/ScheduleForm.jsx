...
const ScheduleForm = () => {
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`https://dhodonto.ctdprojetos.com.br/consulta`,
    {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${mainState.auth}`
      },
      method: "POST",
      body: JSON.stringify({
        "paciente":{
      ...
      console.log(res);
        if(res.status === 200){
          ...
          throw Error("Não foi possivel marcar a consulta! tente noutra hora");
        }else if(res.status === 403){
          if(mainState.auth === ""){
            throw Error("Acesso negado! Por favor logue para pode marcar uma consulta");
          }else{
            changeContext({
                state: 'auth',
                auth: ''
      ...
      >
        ...
        // está em dark mode e deverá utilizar o css correto */}
            <button
              ...
              className={(mainState.theme === "dark") ? `btn btn-secondary` : `btn btn-light`}
              type="submit"
            >
              Schedule
            </button>
